...
const WriteQuestion: React.FC<WriteQuestionProps> = ({ onAdd }) => {
    ...
    useEffect(() => {
        ...
        const redraw = () => {
            ...
            for (let i = 0; i < clickX.current.length; i++) {
                ...
                if (clickDrag.current[i] && i) {
                    context.moveTo(
                        clickX.current[i - 1] * width,
                        clickY.current[i - 1] * height
                    )
                } else {
                    context.moveTo(
                        clickX.current[i] * width - 1,
                        clickY.current[i] * height
                    )
                }
                context.lineTo(
                    clickX.current[i] * width,
                    clickY.current[i] * height
                )
                context.stroke()
            }
        ...
        const pointerMoveHandler = (ev: PointerEvent) => {
            if (
                !paint.current ||
                (ev.pointerType !== 'mouse' && ev.pointerType !== 'touch')
            )
                return
            const rect = canvas.getBoundingClientRect()
            const mouseX = (ev.clientX - rect.left) / rect.width
        ...
        const pointerUpHandler = () => {
            paint.current = false
            const shape = recognizeGesture(clickX.current, clickY.current)
    ...
    return (
        <div>
            ...
            />
            Dessinez une forme ici ⬆️ (ligne verticale ou horizontale, chevron
            droit ou gauche)
